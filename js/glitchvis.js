...
d3.json("data/clusterdata.json", function(error, graph) { //this is in the data folder
              ...
              sankey //nodes, links, and layouts are in the sankey.js folder
                  .nodes(graph.nodes)
                  .links(graph.links)
                  .layout(32);
...
// add in the title for the nodes
  node.append("text")
      ...
      .style("font-size", 5) //figure out why this doesn't work!?!?!?!
...
function onclick(d){
  d3.selectAll(".link")
      .style("stroke-opacity",function(i){
        if (i.source.node == d.node || i.target.node == d.node) return 0.5; //  code info (&& = and || = or)
      });
